...
class Controladorproducto
{
    ...
    static public function ctrMostrarobjeto($item, $valor)
    {
        ...
        $tabla1 = "producto";
        $tabla2 = "comida";
        $datos=$_GET["Cod_producto"];
        if($_GET["FotoCo"] != ""){
                unlink($_GET["FotoCo"]);
                rmdir('views/img/productos/'.$_GET["Cod_producto"]);
           }
        $respuesta=Modeloproducto::mdlBorrarProducto($tabla1,$tabla2,$datos); 
    ...
    static public function ctrBorrarObjeto(){
        if(isset($_GET["Cod_producto"])){    
        $tabla = "stock";
        $tabla1 = "producto";
        $tabla2 = "objeto_piscina";
        ...
        if($_GET["FotoO"] != ""){
                unlink($_GET["FotoO"]);
                rmdir('views/img/objetos_piscina/'.$_GET["Cod_producto"]);
           }
        $respuesta=Modeloproducto::mdlBorrarObjeto($tabla1,$tabla2,$datos); 
    ...
    public function ctrEditarProducto(){
        if(isset($_POST["editarnombreprod"])){
               if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarnombreprod"])&&
                preg_match('/^[a-zA-Z0-9]+$/', $_POST["editarCantidad"])&&
                preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarProveedor"])&&
                preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarprecioC"])&&
                preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarprecio"])
                )
             $ruta=$_POST["FotoCo"];
            if(isset($_FILES["editarfotoP"]["tmp_name"])){
